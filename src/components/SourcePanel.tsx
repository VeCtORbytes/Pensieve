...
export default function SourcePanel({
  ...
  return (
    <div
      ...
      onDrop={handleDrop}
      className={`relative flex h-full flex-col bg-[#F5F7F8] text-[#141A22] border-r border-[#E2E7EA] transition ${
        isDraggingOver ? "ring-2 ring-[#3B4CC0] bg-[#3B4CC0]/5" : ""
      }`}
    >
      {/* Quiet Rail Header */}
      <div className="flex items-center justify-between px-4 py-3 border-b border-[#E2E7EA] bg-[#F5F7F8]">
        ...
        <button
          ...
          className="p-1.5 text-xs text-[#141A22] hover:bg-white rounded-lg transition cursor-pointer border border-transparent hover:border-[#E2E7EA]"
          title="Add Source"
        >
          <Plus className="w-4 h-4 text-[#3B4CC0]" />
        </button>
      </div>
      ...
      <div className="flex-1 overflow-y-auto px-2 py-3 space-y-1.5">
        {sources.length === 0 ? (
          <div className="py-12 text-center space-y-2">
            <Sparkles className="w-6 h-6 mx-auto text-neutral-400" />
            <p className="text-xs text-neutral-500">No sources in this vessel.</p>
            <p className="text-[10px] text-neutral-400">Drag and drop PDF or VTT files here.</p>
          </div>
        ) : (
          sources.map((s) => (
            <div
              ...
              className="group p-3 rounded-xl bg-white hover:bg-white border border-[#E2E7EA] hover:border-[#3B4CC0] shadow-2xs hover:shadow-md transition cursor-pointer space-y-1.5 text-xs"
            >
              <div className="flex items-center justify-between gap-2">
                <div className="flex items-center gap-2 truncate flex-1">
                  <StatusDot status={s.status} />
                  <span className="font-medium text-[#141A22] truncate group-hover:text-[#3B4CC0] transition">
                    {s.title}
                  </span>
                </div>
                ...
                <div className="flex items-center gap-1 transition md:opacity-0 md:group-hover:opacity-100 md:focus-within:opacity-100">
                  <button
                    ...
                    className="p-1 text-neutral-400 hover:text-[#141A22] rounded transition cursor-pointer"
                  >
              ...
              <div className="flex items-center justify-between gap-2 text-[10px] text-neutral-500 font-mono pl-3.5">
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-xs p-4">
          <div className="bg-white rounded-2xl max-w-xl w-full p-6 shadow-2xl space-y-5 border border-[#E2E7EA] text-[#141A22]">
            <div className="flex items-center justify-between border-b border-[#E2E7EA] pb-3">
              <h3 className="text-base font-serif-display font-normal text-[#141A22] flex items-center gap-2">
                <Sparkles className="w-4 h-4 text-[#3B4CC0]" />
                Add Knowledge Source
              </h3>
              <button
                type="button"
                onClick={handleCloseModal}
                className="text-neutral-400 hover:text-[#141A22] cursor-pointer"
              >
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <form onSubmit={handleCreateSource} className="space-y-4">
              <div>
                ...
                <input
                  ...
                  className="w-full px-3.5 py-2.5 text-xs bg-[#F5F7F8] border border-[#E2E7EA] rounded-xl outline-none focus:ring-2 focus:ring-[#3B4CC0] focus:bg-white text-[#141A22] transition"
                />
              </div>
              ...
              {activeTab === "text" && (
                <div>
                  ...
                  <textarea
                    ...
                    className="w-full px-3.5 py-2.5 text-xs bg-[#F5F7F8] border border-[#E2E7EA] rounded-xl outline-none focus:ring-2 focus:ring-[#3B4CC0] focus:bg-white text-[#141A22] transition font-mono"
                  />
                </div>
              )}
              ...
              {activeTab === "pdf" && (
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-3.5 py-2.5 text-xs bg-[#F5F7F8] border border-[#E2E7EA] rounded-xl text-[#141A22] file:mr-4 file:py-1 file:px-3 file:rounded-lg file:border-0 file:text-xs file:font-semibold file:bg-[#141A22] file:text-white hover:file:bg-[#3B4CC0] transition"
                  />
                </div>
              )}
              ...
              <div className="flex justify-end gap-2 pt-2">
                <button
                  type="button"
                  onClick={handleCloseModal}
                  className="px-4 py-2 text-xs font-semibold text-neutral-500 hover:text-[#141A22] transition"
                >
                  Cancel
                </button>
                <button
                  ...
                  className="flex items-center gap-1.5 px-4 py-2 rounded-xl bg-[#141A22] hover:bg-[#3B4CC0] text-xs font-semibold text-white shadow-sm transition disabled:opacity-50"
                >
                  {isSubmitting ? (
                    <>
...
function StatusDot({ status }: { status: Source["status"] }) {
  if (status === "READY") {
    return <div className="w-2 h-2 rounded-full bg-[#1D9E75] shrink-0" />;
  }
  if (status === "FAILED") {
    return <div className="w-2 h-2 rounded-full bg-red-500 shrink-0" />;
  }
  return <div className="w-2 h-2 rounded-full bg-[#D97706] animate-pulse shrink-0" />;
}
