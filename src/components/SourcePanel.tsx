...
export default function SourcePanel({
  ...
  return (
    <div
      ...
      onDrop={handleDrop}
      className={`relative flex h-full flex-col bg-vessel text-ink border-r border-rule transition ${
        isDraggingOver ? "ring-2 ring-accent bg-accent/5" : ""
      }`}
    >
      {/* Quiet Rail Header */}
      <div className="flex items-center justify-between px-4 py-3 border-b border-rule bg-vessel">
        ...
        <button
          ...
          className="p-1.5 text-xs text-ink hover:bg-white rounded-lg transition cursor-pointer border border-transparent hover:border-rule"
          title="Add Source"
        >
      ...
      <div className="flex-1 overflow-y-auto px-2 py-3 space-y-1.5">
        {sources.length === 0 ? (
          <div className="py-12 text-center space-y-2">
            <FileText className="w-6 h-6 mx-auto text-neutral-400" />
            <p className="text-xs text-neutral-500">No sources yet.</p>
          ...
          sources.map((s) => (
            <div
              ...
              className="group p-3 rounded-xl bg-white hover:bg-white border border-rule hover:border-accent shadow-xs hover:shadow-md transition cursor-pointer space-y-1.5 text-xs"
            >
              <div className="flex items-center justify-between gap-2">
                <div className="flex items-center gap-2 truncate flex-1">
                  <StatusDot status={s.status} />
                  <span className="font-medium text-ink truncate group-hover:text-accent transition">
                    {s.title}
                  </span>
                </div>
                ...
                <div className="flex items-center gap-1 transition md:opacity-0 md:group-hover:opacity-100 md:focus-within:opacity-100">
                  <button
                    ...
                    className="p-1 text-neutral-400 hover:text-ink rounded transition cursor-pointer"
                  >
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-xs p-4">
          <div className="bg-white rounded-2xl max-w-xl w-full p-6 shadow-2xl space-y-5 border border-rule text-ink">
            <div className="flex items-center justify-between border-b border-rule pb-3">
              ...
              <button
                ...
                className="text-neutral-400 hover:text-ink cursor-pointer"
              >
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <form onSubmit={handleCreateSource} className="space-y-4">
              <div>
                ...
                <input
                  ...
                  className="w-full px-3.5 py-2.5 text-xs bg-vessel border border-rule rounded-xl outline-none focus:ring-2 focus:ring-accent focus:bg-white text-ink transition"
                />
              </div>
              ...
              {activeTab === "text" && (
                <div>
                  ...
                  <textarea
                    ...
                    className="w-full px-3.5 py-2.5 text-xs bg-vessel border border-rule rounded-xl outline-none focus:ring-2 focus:ring-accent focus:bg-white text-ink transition font-mono"
                  />
                </div>
              )}
              ...
              {activeTab === "pdf" && (
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-3.5 py-2.5 text-xs bg-vessel border border-rule rounded-xl text-ink file:mr-4 file:py-1 file:px-3 file:rounded-lg file:border-0 file:text-xs file:font-semibold file:bg-ink file:text-white hover:file:bg-accent transition"
                  />
                </div>
              )}
              ...
              <div className="flex justify-end gap-2 pt-2">
                <button
                  type="button"
                  onClick={handleCloseModal}
                  className="px-4 py-2 text-xs font-semibold text-neutral-500 hover:text-ink transition"
                >
                  Cancel
                </button>
                <button
                  ...
                  className="flex items-center gap-1.5 px-4 py-2 rounded-xl bg-ink hover:bg-accent text-xs font-semibold text-white shadow-sm transition disabled:opacity-50"
                >
                  {isSubmitting ? (
                    <>
...
function StatusDot({ status }: { status: Source["status"] }) {
  if (status === "READY") {
    return <div className="w-2 h-2 rounded-full bg-found shrink-0" />;
  }
  if (status === "FAILED") {
    return <div className="w-2 h-2 rounded-full bg-red-500 shrink-0" />;
  }
  return <div className="w-2 h-2 rounded-full bg-gold animate-pulse shrink-0" />;
}
