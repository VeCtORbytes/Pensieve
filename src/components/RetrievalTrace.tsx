  ...
  return (
    <div className="rounded-xl border border-neutral-200 bg-neutral-50/60 overflow-hidden text-xs transition-all duration-200">
      ...
      <button
        ...
        className="w-full px-3.5 py-2 flex items-center justify-between bg-neutral-100/70 hover:bg-neutral-100 font-mono text-neutral-700 transition cursor-pointer select-none"
      >
        <span className="flex items-center gap-2 font-medium">
          <Layers className={`w-3.5 h-3.5 ${stage < 3 ? "animate-pulse text-[#3B4CC0]" : "text-neutral-500"}`} />
          {label}
        </span>
        <span className="flex items-center gap-2 text-[10px] text-neutral-500">
          cutoff {trace.floor.toFixed(2)}
      ...
      {open && (
        <div className="p-3.5 space-y-1 bg-white border-t border-neutral-200/80 animate-in fade-in duration-150">
          {shown.map((c, i) => (
            <div key={i} className="flex items-center gap-2 text-[11px]">
              <span className="w-24 truncate text-neutral-600 font-medium">{c.title}</span>
              {c.humanLocator && (
              ...
              <div className="flex-1 h-2 bg-neutral-100 rounded-full relative overflow-hidden">
                ...
                />
                <div
                  className="absolute top-0 bottom-0 w-0.5 bg-[#1D9E75] transition-opacity duration-300 z-10"
