...
export default function IngestionPipelineVisualizer({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-xs p-4 animate-in fade-in duration-200">
      <div className="bg-white rounded-3xl max-w-3xl w-full p-6 sm:p-8 shadow-2xl border border-[#E2E7EA] text-[#141A22] flex flex-col max-h-[90vh] overflow-hidden">
        {/* Modal Header */}
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-lg font-serif-display font-normal text-[#141A22]">
                RAG & Vector Pipeline Architecture Visualizer
              </h2>
              <p className="text-[11px] text-neutral-500">
                Interactive step-by-step visual demonstration of ingestion, vectorization, & retrieval
              </p>
            </div>
          </div>
        ...
        {/* Step Progress Nodes */}
        <div className="grid grid-cols-5 gap-2 pt-4 pb-2 border-b border-[#E2E7EA]">
            ...
            return (
              <button
                ...
                className={`p-3 rounded-2xl border text-left transition cursor-pointer flex flex-col justify-between space-y-2 relative overflow-hidden ${
                  isActive
                    ? "bg-white border-[#3B4CC0] shadow-md ring-2 ring-[#3B4CC0]/20"
                    : isCompleted
                    ? "bg-[#F5F7F8] border-[#1D9E75]/40 text-neutral-600"
                    : "bg-[#F5F7F8] border-[#E2E7EA] text-neutral-400"
                }`}
              >
                {isActive && (
                  <div className="absolute top-0 left-0 right-0 h-1 bg-[#3B4CC0] animate-pulse" />
                )}
        ...
        {/* Interactive Dynamic Stage Stage Graphic Canvas */}
        <div className="flex-1 my-4 p-6 bg-[#F5F7F8] rounded-3xl border border-[#E2E7EA] flex flex-col justify-between overflow-y-auto space-y-4 relative">
          {/* Header Info */}
          <div className="flex items-start justify-between gap-4 border-b border-[#E2E7EA] pb-3">
            <div>
              <div className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full bg-white border border-[#E2E7EA] text-[10px] font-semibold text-[#3B4CC0] uppercase tracking-wider">
                Stage {currentStep} of 5
              </div>
              <h3 className="text-base font-serif-display font-normal text-[#141A22] mt-1">
            ...
            <div className="space-y-4 py-2 animate-in fade-in zoom-in-95 duration-200">
              ...
              <div className="p-4 bg-white rounded-2xl border border-emerald-200 shadow-2xs space-y-3">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-xs font-mono">
                  <div className="p-2.5 bg-[#F5F7F8] rounded-xl border border-emerald-200 flex items-center justify-between">
                    <span className="text-neutral-700">Point ID: #q18f2a</span>
                    <span className="text-[#1D9E75] font-semibold">Payload Attached ✓</span>
                  </div>
                  <div className="p-2.5 bg-[#F5F7F8] rounded-xl border border-emerald-200 flex items-center justify-between">
          ...
          {currentStep === 4 && (
            <div className="space-y-4 py-2 animate-in fade-in zoom-in-95 duration-200">
              ...
              <div className="p-4 bg-white rounded-2xl border border-amber-200 shadow-2xs space-y-2 text-xs">
                ...
                {[
                  ...
                  <div
                    key={idx}
                    className="p-2.5 bg-[#F5F7F8] rounded-xl border border-[#E2E7EA] flex items-center justify-between"
                  >
                    <span className="font-medium text-[#141A22] truncate max-w-xs">{item.title}</span>
                    <div className="flex items-center gap-2">
          ...
          {currentStep === 5 && (
            <div className="space-y-4 py-2 animate-in fade-in zoom-in-95 duration-200">
              ...
              <div className="p-4 bg-white rounded-2xl border border-indigo-200 shadow-2xs space-y-2.5">
                <div className="flex items-center gap-2 text-xs font-semibold text-indigo-700">
                  <Sparkles className="w-4 h-4 text-indigo-600" />
                  <span>Streamed Fact-Grounded Output</span>
                </div>
                <p className="text-xs text-[#141A22] leading-relaxed font-sans font-medium bg-[#F5F7F8] p-3 rounded-xl border border-[#E2E7EA]">
                  Quantum computing harnesses quantum bits or qubits <span className="inline-flex items-center px-1.5 py-0.5 rounded text-[10px] font-bold text-[#1D9E75] bg-[#1D9E75]/10 border border-[#1D9E75]/30">[1]</span>. Key principles include superposition and entanglement <span className="inline-flex items-center px-1.5 py-0.5 rounded text-[10px] font-bold text-[#1D9E75] bg-[#1D9E75]/10 border border-[#1D9E75]/30">[2]</span>.
                </p>
              </div>
            </div>
          )}
          ...
          {/* Navigation Bar */}
          <div className="flex items-center justify-between pt-2 border-t border-[#E2E7EA]">
            <button
              type="button"
              ...
              className="px-4 py-2 rounded-xl bg-white border border-[#E2E7EA] hover:border-[#3B4CC0] text-xs font-semibold text-[#141A22] transition disabled:opacity-40 cursor-pointer shadow-2xs"
            >
              Back
            </button>

            <span className="text-xs font-mono text-neutral-500">
              Step <strong className="text-[#3B4CC0]">{currentStep}</strong> of 5
            </span>
            ...
            <button
              ...
              className="flex items-center gap-1 px-4 py-2 rounded-xl bg-[#141A22] hover:bg-[#3B4CC0] text-xs font-semibold text-white transition cursor-pointer shadow-xs"
            >
